...
export const createFormSteps = (farmBlocks, countries, produceCategories, crops, grades, stores) => [
  {
  ...
  },
  {
    ...
    fields: [
      ...
      // ── NEW: Crop — dépend de la catégorie ────────────────────────────────
      {
        type: "select", name: "crop_id", label: "Crop", required: true,
        options: crops.map((c) => ({ value: String(c.id), label: c.name })),
        disabled: crops.length === 0,
        placeholder: crops.length === 0 ? "Select a category first" : "Select a crop",
      },
      // ── Grade — dépend du crop ────────────────────────────────────────────
      {
        type: "select", name: "Crop_grade", label: "Crop Grade", required: true,
        options: grades.map((g) => ({
          value: g.grade_value,
          label: `${g.grade_value}${g.description ? ` — ${g.description}` : ""}`,
        })),
        disabled: grades.length === 0,
        placeholder: grades.length === 0 ? "Select a crop first" : "Select a grade",
      },
    ],
  },
  {
