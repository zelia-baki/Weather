      ...
      try {
        ...
        if (response.data.status === "success") {
          ...
          if (geolocation && geolocation.includes(",")) {
            ...
            setLatitude(latNum);
            setLongitude(lonNum);
        // ✅ Récupérer les points de la ferme pour calculer l’aire
          const pointsRes = await axiosInstance.get(`/api/points/getbyownerid/farm/${farmId}`);
          ...
          if (points.length > 2) {
            ...
            setLandSurface(Math.round(area));
          } else {
            setLandSurface(0); // pas assez de points
          }
            if (mapRef.current) {
