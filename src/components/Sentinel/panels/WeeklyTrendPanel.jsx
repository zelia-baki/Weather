...
// ─────────────────────────────────────────────────────────────────────────────
// Chart.js plugin: draws a greyed-out band behind each week with no usable
// Sentinel-2 image (too cloudy). Makes the difference between "low value"
// and "no data" visually unambiguous — the two must never look the same.
// ─────────────────────────────────────────────────────────────────────────────
const noDataBandsPlugin = {
  ...
  beforeDraw(chart, _args, opts) {
    ...
    hasData.forEach((has, i) => {
      ...
      // Light diagonal hatching to reinforce readability in print/PDF
      ...
      tooltip: {
        ...
        callbacks: {
          label: ctx => {
            ...
            if (!has) return null; // avoid showing "null" in the tooltip
