 ...
 * CarbonForestPDF.jsx
 * ──────────────────────────────────────────────────────────────────────────
 * Template PDF pour le Carbon Report d'une FORÊT.
 * Ce composant est CACHÉ à l'écran (left: -9999px).
 * Largeur fixe 794 px = A4 @ 96 dpi → PDF toujours identique quel que soit l'écran.
 *
 * Différences forest vs farm :
 *  - entityLabel : "Forest"
 *  - info affichée: name, tree_type, date_created
 *  - PAS de farm_id, crops, geolocation, land_type
 *  - API : /api/gfw/forest/${forestId}/CarbonReport
 */
...
// ── Composant ────────────────────────────────────────────────────────────────
const CarbonForestPDF = ({ pdfRef, forestInfo, geoData, mapboxUrl, areaInHectares, areaInSquareMeters }) => {
    ...
    >
      ...
      {forestInfo && (
        <div style={{ ...sectionBox, marginBottom: '24px' }}>
          <div style={sectionTitle}>Forest Information</div>
          <table style={table}>
            <tbody>
              <tr>
                <td style={tdAlt}><strong>Forest Name</strong></td>
                <td style={tdStyle}>{forestInfo.name}</td>
                <td style={tdAlt}><strong>Tree Type</strong></td>
                <td style={tdStyle}>{forestInfo.tree_type ?? 'N/A'}</td>
              </tr>
              <tr>
                <td style={tdAlt}><strong>Date Created</strong></td>
                <td style={tdStyle}>{forestInfo.date_created ?? 'N/A'}</td>
                <td style={tdAlt}><strong>Last Updated</strong></td>
                <td style={tdStyle}>{forestInfo.date_updated ?? 'N/A'}</td>
              </tr>
              {areaInHectares && (
                <tr>
      ...
      <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start', margin: '24px 0' }}>
        ...
        <div style={{
          ...
          <p>
            <strong>Gross Emissions</strong> represent carbon released through land-use change and forest disturbances.
          </p>
          <p>
            <strong>Gross Removals</strong> represent carbon absorbed through forest growth and regeneration.
          </p>
          <p>
            <strong>Net Flux</strong> is the balance: negative = net carbon
            <span style={{ color: '#43a047', fontWeight: 700 }}> sink</span>; positive = net
...
export default CarbonForestPDF;
